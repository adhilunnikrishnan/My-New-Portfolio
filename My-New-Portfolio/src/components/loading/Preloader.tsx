  ...
  return (
    <div className="fixed inset-0 w-screen h-screen flex items-end justify-center z-[9999] bg-transparent pointer-events-none">
      {heights.map((h, i) => (
        ...
        >
          {i === 29 && (
            <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center justify-center z-10 pointer-events-none text-background font-accent text-[1.5vw] uppercase select-none whitespace-nowrap bottom-[calc(60%-0.7vw)] leading-[0.9] tracking-[0.05em]">
              {name1.map((char, index) => <span key={index} className="opacity-0 block translate-y-[10px]">{char}</span>)}
            </div>
          )}
          {i === 30 && (
            <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center justify-center z-10 pointer-events-none text-background font-accent text-[1.5vw] uppercase select-none whitespace-nowrap top-[calc(40%-0.7vw)] leading-[0.9] tracking-[0.05em]">
      ...
      <div className="absolute top-1/2 right-20 -translate-y-1/2 z-[10001] pointer-events-none">
      ...
      <div className="absolute bottom-0 left-0 w-full h-20 flex items-end z-[10000] overflow-hidden">
        ...
        <span className="absolute right-8 bottom-3 color-background font-display font-extrabold text-[2.2rem] tabular-nums loading-percentage">0%</span>
      </div>
    </div>
  );
};
